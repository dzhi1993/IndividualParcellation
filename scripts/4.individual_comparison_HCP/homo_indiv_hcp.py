...
from global_config import (ATLAS_DIR, BASE_DIR, DEVICE, ERIS_DIR, MODEL_DIR,
                           RANDY_DIR, REPLICATION_DIR, RESULTS_DIR,
                           SUBJECT_LIST_DIR)
...
if __name__ == "__main__":
    ...
    t_data = ut.build_resting_data('RANDY15', space='fs32k',
                                  ses_list=[f'ses-rest{sn}' for sn in range(6, 11)],
                                  type='Tseries', subj=None, hemis=None, smooth=None)
        ...
        for counter_p, p in enumerate(group_strength_list):
            for counter_w, w in enumerate(spatial_list):
                ...
                Pindiv = nb.load(MODEL_DIR + f'/Models_03/indiv_parcellation/RANDY15_test_set' +
                        f'/asym_HBPHc+RANDYrest-{global_counter}run-indiv_space-fs32k_K-15_Ico642Run_groupstrengh-{p}_spatial-{w}.dlabel.nii').get_fdata()
                ...
                #                  '/DU15_fs32k_individual_parcellation_wb.dlabel.nii').get_fdata()
                ...
                for r, td in enumerate(t_data):
                    ...
                    for task in tasks_list:
                        if task == 'all':
                            ...
                            ## rest homogeneity (per network)
                            # homo_nets = ev.calc_test_homogeneity(Pindiv, td[:, idx, :], return_single=False)
                            # homo_nets = pt.where(homo_nets == 0, pt.nan, homo_nets)
                            # np.save(MODEL_DIR + f'/Models_03/indiv_parcellation/RANDY15_test_set/homogeneity' +
                            #         f'/homo_nets_asym_MdNiIbHc+RANDYrest-{global_counter}run-indiv_K-{K}_strengh-{p}_spatial-{w}_Tseries_sm2_11sub.npy',
                            #         homo_nets.cpu().numpy())
                        else:
