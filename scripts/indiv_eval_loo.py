...
if __name__ == "__main__":
    ...
    #             indiv_par = atlas.cifti_to_data(f'/home/dzhi/eris_mount/dzhi/workspace/DU15NET/Harvard_Precision_Neuroimaging_Group_gz_zK166' +
    #                     f'/DU15_fs32k_individual_parcellation_wb.dlabel.nii')
    ...
    #                     f'/inhomo_nets_DU-indiv_K-{K}.npy',
    ...
    #
    # results.to_csv(f'eval_DU_indiv_k-15_test_on_RANDYtask-contrast.tsv', index=False, sep='\t')
    ...
    cond_vec = [np.arange(1,1211)] * 24
    part_vec = [np.repeat(np.array([1]), 1210)] * 24
    subj_ind = [np.arange(15)] * 24
    ...
    U = atlas.cifti_to_data(ERIS_DIR + '/dzhi/workspace/res/priors/MSHBM_group_prior_HCP40training_k-15.dscalar.nii')
    ...
    for counter_p, p in enumerate([0.1,1,10,100]):
        for counter_w, w in enumerate([0,0.5,1,2,5]):
            print(f'prior strength is {p}; the MRF strength is {w} ...')
            # m-RBM
            ar_model = ar.build_arrangement_model(U*p, prior_type='prob', atlas=atlas,
            ...
            nb.save(img, MODEL_DIR + f'/Models_04/indiv_parcellation/RANDY15_test_set' +
                    f'/asym_MSHBM40+RANDYrest-allrun-indiv_space-fs32k_K-{K}_{fc_type}_groupstrengh-{p}_spatial-{w}.dlabel.nii')
            ...
            minfo = ge.make_eval_info(K, train_info=['RANDY15'], train_sess='half-1',
                                        ...
                                        model_type='Models_04', group_map_name='MSHBM40',
                                        test_kappa=None)
            ...
            for r, td in enumerate(t_data):
                ...
                for task in tasks_list:
                    ...
                    np.save(MODEL_DIR + f'/Models_04/indiv_parcellation/RANDY15_test_set/zvalues' +
                            f'/zvalue_indiv_asym_MSHBM40+RANDYrest-allrun-indiv_K-{K}_strengh-{p}_spatial-{w}.npy',
                            zvalue_indiv.cpu().numpy())
                    inhomo_nets = ev.calc_test_task_inhomogeneity(Pindiv, td[:, idx, :], return_single=False)
                    inhomo_nets = pt.where(inhomo_nets == 0, pt.nan, inhomo_nets)
                    np.save(MODEL_DIR + f'/Models_04/indiv_parcellation/RANDY15_test_set/inhomogeneity' +
                            f'/inhomo_nets_asym_MSHBM40+RANDYrest-allrun-indiv_K-{K}_strengh-{p}_spatial-{w}.npy',
                            inhomo_nets.cpu().numpy())
    ...
    results.to_csv(
        RES_DIR + f'/eval_MSHBM40+RANDYrest-allrun_K-{K}_indiv-independent_test_on_RANDYtask-contrast_hemis-L.tsv',
        index=False, sep='\t')
    print('Done')
