...
if __name__ == "__main__":
    ...
    #     # ## HCP resting data
    #     # data, cond_vec, part_vec, subj_ind, rs_info = gp.build_hcp_datasets(HCP_DIR,
    #     #                                                                     f"subj_list/HCP203_test_set_filtered_{global_counter}.tsv",
    #     #                                                                     atlas, ses_list=['ses-rest1'],
    #     #                                                                     join_sess=False, join_sess_part=False,
    #     #                                                                     part_ind='run', part_num=None,
    #     #                                                                     cond_ind=['net_id'],
    #     #                                                                     type=['Ico642Run'], hemis=None,
    ...
    #
    #     ## HCP task data
    #     data2, cond_vec2, part_vec2, subj_ind2, t_info = gp.build_hcp_datasets(HCP_DIR, f'subj_list/HCP203_test_set_filtered_1.tsv',
    ...
    #
    #     data2 = [np.concat(data2, axis=1)]
    #     cond_vec2 = [np.concat(cond_vec2)]
    #     part_vec2 = [np.concat(part_vec2)]
    #     subj_ind2 = [subj_ind2[0]]
    #     n_subj = np.unique(np.concatenate(subj_ind2, axis=0)).size
    #
    #     toc = time.perf_counter()
    #     print(f'Done loading. Used {toc - tic:0.4f} seconds!')
    #     hut.report_cuda_memory()
    #
    #     for task_out in hcp_tasks:
    #         task_names = hcp_tasks.copy()
    #         task_names.remove(task_out)
    #         this_ind = t_info["task_name"].isin(task_names)
    #         data = [dat[:,this_ind,:] for dat in data2]
    #         cond_vec = [cond[this_ind] for cond in cond_vec2]
    #         part_vec = [part[this_ind] for part in part_vec2]
    #         subj_ind = subj_ind2
    ...
    #                                         column_names=T.participant_id.tolist(),
    #                                         label_names=net_name, label_RGBA=colors)
    #                 nb.save(img, MODEL_DIR + f'/Models_03/indiv_parcellation/HCP203_test_set/leave_one_out' +
    #                         f'/asym_MdNiIbWmDeSoHc+HCPtask2run-wo{task_out}-indiv_space-fs32k_K-{K}_{fc_type}_groupstrengh-{p}_spatial-{w}_1.dlabel.nii')
    ...
    results = pd.DataFrame()
    for global_counter in hcp_tasks:
        for counter_p, p in enumerate(group_strength_list):
            for counter_w, w in enumerate(spatial_list):
                print(f'Evaluating {global_counter} runs data, prior strength {p}, the spatial w {w} ...')
                # Load indiv parcellation
                Pindiv = nb.load(MODEL_DIR + f'/Models_03/indiv_parcellation/HCP203_test_set/leave_one_out' +
                        f'/asym_MdNiIbWmDeSoHc+HCPtask2run-wo{global_counter}-indiv_space-fs32k_K-{K}_{fc_type}_groupstrengh-{p}_spatial-{w}_1.dlabel.nii').get_fdata()
                # Load Kong2019 individual parcellations
                Pindiv_du = ut.get_kong2019_indiv_parcellations(ERIS_DIR + '/dzhi/workspace/res/ind_parcellation/HCP203_test_set',
                ...
                for r, td in enumerate(t_data):
                    ...
                    for task in tasks_list:
                        ...
                        np.save(MODEL_DIR + f'/Models_03/indiv_parcellation/HCP203_test_set/leave_one_out/zvalues' +
                                f'/zvalue_indiv_asym_MdNiIbWmDeSoHc+HCPtask2run-wo{global_counter}-indiv_K-{K}_strengh-{p}_spatial-{w}_sm4.npy',
                        ...
                        np.save(MODEL_DIR + f'/Models_03/indiv_parcellation/HCP203_test_set/leave_one_out/inhomogeneity' +
                                f'/inhomo_nets_asym_MdNiIbWmDeSoHc+HCPtask2run-wo{global_counter}-indiv_K-{K}_strengh-{p}_spatial-{w}_sm4.npy',
    ...
    for do in ['EPROJ','MOTOR','NBACK','VODDK','LANG', 'TOM', 'VISME']:
        res = np.load(MODEL_DIR + '/Models_03/indiv_parcellation/RANDY15_test_set/leave_one_out/zvalues' +
                      f'/zvalue_indiv_asym_MdNiIbHc+RANDYrest5run+task5run-wo{do}-indiv_K-15_strengh-1_spatial-1_sm2.npy')
