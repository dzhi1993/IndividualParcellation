...
if __name__ == "__main__":
    ...
    ext = '_binarized'
    K=15
    group_strength_list = [1,2,5,10,20]
    ...
    for index, row in T.iterrows():
        ######## Step 1. Load subjects individual training data
        print(f'Start loading data {index}: RANDY resting - {training_ses}, {fc_type} {ext} ...')
        ...
        data = ut.build_resting_data('RANDY15', space='fs32k',
                                     ses_list= [f'ses-rest{sn}' for sn in range(1, row['num_fix_run'] + 1)],
                                     type='Ico642Run', subj=row["participant_id"], hemis=None, smooth=None)

        # ## RANDY15 task data
        # data, cond_vec, part_vec, subj_ind = ut.build_dataset('RANDY15', atlas=atlas.name,
        #                                                         sess=['ses-task'], cond_ind='reg_id',
        #                                                         type='CondRun', part_ind='sn',
        #                                                         part_num=np.arange(1,25), join_sess=False,
        #                                                         join_sess_part=False, smooth=None)

        cond_vec = [np.arange(1,1211)] * len(data)
        part_vec = [None] * len(data)
        subj_ind = [np.array([0])] * len(data)
        ...
        for counter_p, p in enumerate(group_strength_list):
            for counter_w, w in enumerate(spatial_list):
                ...
                U_indiv, _, M = fm.get_indiv_parcellation(ar_model, atlas, data,
                                                        ...
                                                        sym_type='asym', n_iter=20 if ar_model.name == 'cRBM_Wc' else 200,
                ...
                img = nt.make_label_cifti(Pindiv.T.cpu().numpy(), atlas.get_brain_model_axis(),
                                        column_names=[row["participant_id"]],
                                        label_names=net_name, label_RGBA=colors)
                nb.save(img, MODEL_DIR + f'/Models_03/indiv_parcellation/RANDY15_test_set' +
                        f'/asym_MSHBM40+RANDYrest-allrun-indiv_space-fs32k_K-{K}_{fc_type}_groupstrengh-{p}_spatial-{w}_{row["participant_id"]}.dlabel.nii')
    ...
    print(f'Done loading. Used {toc - tic:0.4f} seconds!')
    hut.report_cuda_memory()
    n_subj = t_data[0][randy_good_subjlist,:,:].shape[0]

    ## Making distance metric
    dist = pt.load(BASE_DIR + '/Atlases/tpl-fs32k/distGOD_fs32k.pt', weights_only=True)

    results = pd.DataFrame()
    for global_runs in global_run_list:
        for counter_p, p in enumerate(group_strength_list):
            for counter_w, w in enumerate(spatial_list):
                print(f'Evaluating {global_runs} runs data, prior strength {p}, the spatial w {w} ...')
                # Load indiv parcellation
                Pindiv = nb.load(MODEL_DIR + f'/Models_03/indiv_parcellation/RANDY15_test_set' +
                        f'/asym_MSHBM40+RANDYrest-{global_runs}run-indiv_space-fs32k_K-15_Ico642Run_groupstrengh-{p}_spatial-{w}.dlabel.nii').get_fdata()
                # Load Kong2019 individual parcellations
                ...
                # Pindiv = nb.load(ERIS_DIR + '/dzhi/workspace/DU15NET/Harvard_Precision_Neuroimaging_Group_gz_zK166' +
                ...
                minfo = ge.make_eval_info(K, train_info=['HCP'], train_sess='run-2',
                                            ...
                                            model_type='Models_03', group_map_name='MSHBM40',
                                            test_kappa=None)
                ...
                for r, td in enumerate(t_data):
                    ...
                    for task in tasks_list:
                        ...
                        np.save(MODEL_DIR + f'/Models_03/indiv_parcellation/RANDY15_test_set/zvalues' +
                                f'/zvalue_indiv_asym_MSHBM40+RANDYrest-{global_runs}run-indiv_K-{K}_strengh-{p}_spatial-{w}_sm2.npy',
                        ...
                        np.save(MODEL_DIR + f'/Models_03/indiv_parcellation/RANDY15_test_set/inhomogeneity' +
                                f'/inhomo_nets_asym_MSHBM40+RANDYrest-{global_runs}run-indiv_K-{K}_strengh-{p}_spatial-{w}_sm2.npy',
                                inhomo_nets.cpu().numpy())

                        ### Rest homogeneity / task inhomogeneity (global)
                        # homo_indiv = ev.calc_test_homogeneity(Pindiv, td[:,idx,:])
                        inhomo_indiv = ev.calc_test_task_inhomogeneity(Pindiv[randy_good_subjlist,:], td[randy_good_subjlist][:,idx,:], return_single=True)

                        ## Factorize DCBC calcuation
                        hut.report_cuda_memory()
                        dcbc_indiv = ev.calc_test_dcbc(Pindiv[randy_good_subjlist,:], td[randy_good_subjlist][:,idx,:], dist, trim_nan=True)
                        pt.cuda.empty_cache()
                        hut.report_cuda_memory()
                ...
                this_res['train_runs'] = global_runs
    ...
    res2 = np.load(MODEL_DIR + '/Models_03/indiv_parcellation/RANDY15_test_set/zvalues' +
                   f'/zvalue_indiv_asym_HBPHc+RANDYrest-{num_run}run-indiv_K-15_strengh-1_spatial-1_sm2.npy')
