...
if __name__ == "__main__":
    ...
    spatial_list = [0]
    global_run_list = np.arange(15)+1
    ...
    #                                   ses_list=[f'ses-rest{i}' for i in range(global_counter, global_counter+1)],
    #                                   type='Ico642Run', subj=None, hemis=None, smooth='binarized-0.1')
    ...
    #                     f'/asym_MdNiIbHc+RANDYrest1run{global_counter}-indiv_space-fs32k_K-15_Ico642Run_groupstrengh-{p}_spatial-{w}.npy',
    #                     U_indiv.cpu().numpy())
    #             del M
    #             # ll_all.append(ll)
    #             pt.cuda.empty_cache()
    ...
    #                                     column_names=T.participant_id.tolist(),
    #                                     label_names=net_name, label_RGBA=colors)
    #             nb.save(img, MODEL_DIR + f'/Models_03/indiv_parcellation/RANDY15_test_set/runs' +
    #                     f'/asym_MdNiIbHc+RANDYrest-1run{global_counter}-indiv_space-fs32k_K-15_Ico642Run_groupstrengh-{p}_spatial-{w}.dlabel.nii')
    ...
    for global_counter in global_run_list:
        ...
        for counter_p, p in enumerate(group_strength_list):
            for counter_w, w in enumerate(spatial_list):
                ...
                Pindiv = nb.load(MODEL_DIR + f'/Models_03/indiv_parcellation/RANDY15_test_set/runs' +
                        f'/asym_HBPHc+RANDYrest-1run{global_counter}-indiv_space-fs32k_K-15_Ico642Run_groupstrengh-{p}_spatial-{w}.dlabel.nii').get_fdata()
                ...
                minfo = ge.make_eval_info(K, train_info=['RANDY'], train_sess=f'run-{global_counter}',
                                            ...
                                            model_type='Models_03', group_map_name='HBPHc',
                                            test_kappa=None)
                    ...
                    for task in tasks_list:
                        if task == 'all':
                            ...
                            np.save(MODEL_DIR + f'/Models_03/indiv_parcellation/RANDY15_test_set/soft_dice' +
                                    f'/soft_dice_indiv_asym_HBPHc+RANDYrest-1run{global_counter}-indiv_K-{K}_strengh-{p}_spatial-{w}_sm2_11sub_pos-z.npy',
                            ...
                            #         f'/zvalue_indiv_asym_HBPHc+RANDYrest-1run{global_counter}-indiv_K-{K}_strengh-{p}_spatial-{w}_sm2_11sub.npy',
                            ...
                            #         f'/inhomo_nets_asym_HBPHc+RANDYrest-1run{global_counter}-indiv_K-{K}_strengh-{p}_spatial-{w}_sm2_11sub.npy',
