...
from depreciated.full_model_symmetric import FullModel

REPO_ROOT = Path(__file__).resolve().parents[2]
RESULT_DIR = REPO_ROOT / 'results' / 'result_1'
...
def plot_evaluation(D, criteria=['uerr', 'cos_err', 'Ecos_err', 'dcbc_group', 'dcbc_indiv'],
    ...
    plt.savefig(result_path('test_errs.pdf'), format='pdf')
    plt.show()
...
if __name__ == '__main__':
    ...
    # Supp Fig. 2b,c,d
    output_file = result_path('eval_cpmRBM_fit.tsv')
