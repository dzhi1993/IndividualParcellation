...
if __name__ == "__main__":
    ...
    for global_counter in [1]:
        subj_list_file = f"HCP200_test.tsv"
        ...
        # ## HCP resting data
        # data2, cond_vec2, part_vec2, subj_ind2, rs_info = gp.build_hcp_datasets(HCP_DIR, f"subj_list/{subj_list_file}",
        #                                             atlas, ses_list=['ses-rest1'],
        #                                             join_sess=False, join_sess_part=False,
        #                                             part_ind='run', part_num=[1,2], cond_ind=['net_id'],
        #                                             type=['Ico642Run'], hemis=None, smooth='4fwhm_binarized')
        ## HCP task test data
        t_data, cond_vec, part_vec, subj_ind, t_info = ut.build_hcp_datasets(HCP_DIR, f"/subj_list/{subj_list_file}",
                                                        atlas, ses_list=['ses-task'], join_sess=False,
        ...
        cond_vec = [con[contrast_idx] for con in cond_vec]
        part_vec = [part[contrast_idx] for part in part_vec]
        ...
        for half in [1,2]:
            ...
            for counter_p, p in enumerate(group_strength_list):
                for counter_w, w in enumerate(spatial_list):
                    ...
                    U_indiv_rest = np.load(MODEL_DIR + f'/Models_03/indiv_parcellation/HCP200_test_set/prob' +
                            f'/asym_MdNiIbHc+HCPrest-1run1-indiv_space-fs32k_K-17_Ico642Run_groupstrengh-10_spatial-5.npy')
                    U_indiv_rest = pt.tensor(U_indiv_rest, dtype=pt.get_default_dtype())

                    U_indiv_rest2261s = np.load(MODEL_DIR + f'/Models_03/indiv_parcellation/HCP200_test_set/prob' +
                                           f'/asym_MdNiIbHc+HCPrest-2261s-indiv_space-fs32k_K-17_Ico642Run_groupstrengh-50_spatial-1.npy')
                    U_indiv_rest2261s = pt.tensor(U_indiv_rest2261s, dtype=pt.get_default_dtype())

                    U_indiv_task = np.load(MODEL_DIR + f'/Models_03/indiv_parcellation/HCP200_test_set/prob' +
                            f'/asym_MdNiIbHc+HCPtask-half{half}-sm6-zstat-masked_K-17_groupstrengh-1_spatial-1.npy')
                    U_indiv_task = pt.tensor(U_indiv_task, dtype=pt.get_default_dtype())

                    U_indiv_fusion = np.load(MODEL_DIR + f'/Models_03/indiv_parcellation/HCP200_test_set/prob' +
                            f'/asym_MdNiIbHc+HCPrest-1run1+task-half{half}-sm6-zstat-masked-indiv_space-fs32k_K-17_Ico642Run_groupstrengh-1_spatial-1.npy')
                    ...
                    minfo = ge.make_eval_info(K, train_info=['HCP'], train_sess=half,
                                              tdata='HCP', test_sess=3-half,
                    ...
                    em_model = em.build_emission_model(K, atlas, 'VMF', hut.indicator(cond_vec[2-half]),
                                                        part_vec[2-half], V=None, em_params=em_params)
                    M_test = fm.FullMultiModel(deepcopy(ar_model), [em_model])
                    coserr_indiv = ev.calc_test_error(M_test, t_data[2-half],
                                                      ['group','floor', U_indiv_rest, U_indiv_rest2261s,
                                                       U_indiv_task, U_indiv_fusion])
                    pt.cuda.empty_cache()
                    hut.report_cuda_memory()
                    ...
                    res['indiv-rest2261s_coserr'] = coserr_indiv[3]
    ...
    results.to_csv(
        HCP_DIR + f'/coserr_indiv-mRBM_MdNiIbHc-HCP200_rest_vs_fusion_K-17_test_on_HCPtask-beta_sm6.tsv',
        index=False, sep='\t')
    print('Done')
    ...
    T = pd.read_csv(get_subject_list_path("HCP40_validation.tsv"), sep='\t')
    # t_data, t_info = ut.load_hcp_contrasts(HCP_DIR, "/subj_list/HCP40_validation.tsv", space='fs32k',
    ...
    ## 3. HCP task betas
    t_data, _, _, _, t_info = ut.build_hcp_datasets(HCP_DIR, f"/subj_list/HCP40_validation.tsv",
    ...
    for counter_p, p in enumerate(group_strength_list):
        for counter_w, w in enumerate(spatial_list):
            ...
            Pindiv = nb.load(MODEL_DIR + f'/Models_03/indiv_parcellation/validation_set' +
                    f'/asym_MdNiIbHc+HCPrest-run1+task-half1-indiv_space-fs32k_K-17_Ico642Run_groupstrengh-{p}_spatial-{w}.dlabel.nii').get_fdata()[:]
            ...
            # Making evaluation information
            minfo = ge.make_eval_info(K, train_info=['HCP'], train_sess='run-1',
            ...
            for r, td in enumerate(t_data):
                ...
                for task in tasks_list:
                    if task == 'all':
                        ...
                        np.save(MODEL_DIR + f'/Models_03/indiv_parcellation/validation_set/zvalues' +
                                f'/zvalue_indiv_asym_MdNiIbHc+HCPrest-run1+task-half1-indiv_K-{K}_strengh-{p}_spatial-{w}_sm6.npy',
                        ...
                        np.save(MODEL_DIR + f'/Models_03/indiv_parcellation/validation_set/inhomogeneity' +
                                f'/inhomo_nets_asym_MdNiIbHc+HCPrest-run1+task-half1-indiv_K-{K}_strengh-{p}_spatial-{w}_sm6.npy',
