...
if __name__ == "__main__":
    ...
    test_ses = 'ses-rest1'
    K=17
    # subj_list_file = "HCP203_test_set_filtered.tsv"
    group_strength_list = [20]
    spatial_list = [100]
        ...
        for train_runs in [2]:
            for counter_p, p in enumerate(group_strength_list):
                for counter_w, w in enumerate(spatial_list):
                    ...
                    # Load Kong2019 individual parcellations
                    Pindiv = ut.get_kong2019_indiv_parcellations(ERIS_DIR + '/dzhi/workspace/res/ind_parcellation/HCP200_test_set',
                    ...
                    for r, td in enumerate(t_data):
                        ...
                        for task in tasks_list:
                            if task == 'all':
                                ...
                                np.save(f'/home/dzhi/eris_mount/dzhi/Indiv_par/Kong_2019/indiv_par/zvalues' +
                                ...
                                np.save(f'/home/dzhi/eris_mount/dzhi/Indiv_par/Kong_2019/indiv_par/inhomogeneity' +
    ...
    ## Mean z-value
    num_run = 1
    d_type = ['yeo2011', 'kong2019', 'run1', 'run2', 'run3', 'run4']
    ...
    YEO2011 = ar.expand_mn_1d(pt.tensor(YEO2011, dtype=pt.long), K=18)
    indx = hev.matching_greedy(align, YEO2011[1:, :].to(pt.get_default_dtype()))

    res_yeo2011 = []
    for subs in [1,2,3,4]:
        res_yeo = np.load('/home/dzhi/eris_mount/dzhi/Indiv_par/Results/section_4/HCP/inhomogeneity' +
                                f'/inhomo_nets_group_YEO2011_on_HCPcontrasts_sm4_{subs}.npy')
        res_yeo2011.append(res_yeo)
    res_yeo2011 = np.concatenate(res_yeo2011, axis=0)
    res_yeo2011 = res_yeo2011[:,indx.cpu().numpy(),:]

    res_kong2019 = []
    for subs in [1,2,3,4]:
        res_kong = np.load('/home/dzhi/eris_mount/dzhi/Indiv_par/Results/section_4/HCP/inhomogeneity' +
                          f'/inhomo_nets_group_KONG2019_on_HCPcontrasts_sm4_{subs}.npy')
        res_kong2019.append(res_kong)
    res_kong2019 = np.concatenate(res_kong2019, axis=0)

    indiv = []
    for run in [1,2,3,4]:
        run_res = []
        for subs in [1,2,3,4]:
            res = np.load(f'/home/dzhi/eris_mount/dzhi/Indiv_par/Kong_2019/indiv_par/inhomogeneity' +
                          f'/inhomo_nets_asym_MSHBM-{run}run-indiv_K-17_strengh-100_spatial-30_sm4_{subs}.npy')
            run_res.append(res)
        run_res = np.concatenate(run_res, axis=0)
        indiv.append(run_res)

    df_all = pd.DataFrame()
    _, t_info = ut.load_hcp_contrasts(HCP_DIR, f"/subj_list/test.tsv", space='fs32k',
                                           hemis=None, smooth='2_MSMAll')
    # t_info['task_name'] = t_info['domain']
    for j, res in enumerate([res_yeo2011, res_kong2019] + indiv):
        for i in np.where(t_info["positive"] == 1)[0]:
            df = plot_zvalues_inhomo(res, i, t_info, type='inhomo', parcel_name=net_name[1:])
            df['dtype'] = d_type[j]
    ...
    for n, c_name in enumerate(con_nam):
        ...
        sb.barplot(data=this_df_all, x='parcel', y='inhomo', hue='dtype', errorbar='se', width=0.7)
