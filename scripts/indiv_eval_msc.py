...
MSC_DIR = '/home/dzhi/eris_mount/Tian/MSC'
if not Path(MSC_DIR).exists():
    MSC_DIR = '/data/tge/Tian/MSC'
if not Path(MSC_DIR).exists():
    raise (NameError('Could not find MSC_DIR'))
...
if __name__ == "__main__":
    ...
    ext = '_binarized'
    K=17
    group_strength_list = [0.5,1,2,5,10,20,30,50]
    spatial_list = [0,0.5,1,2,5,10,15,20]
    msc_tasks = ['motor','memory','mixed']
    ...
    #     subj_list_file = f"participants.tsv"
    ...
    #     ## MSC task betas
    #     data1, cond_vec1, part_vec1, subj_ind1, t_info = ut.build_dataset('MSC', atlas=atlas.name,
    #                                                sess=msc_tasks, cond_ind='reg_id',
    #                                                type='CondRun', part_ind='sn', subj=None,
    #                                                part_num=np.arange(1, global_counter+1), join_sess=True,
    #                                                join_sess_part=False, smooth=None)
    #     ## MSC resting data
    #     data, cond_vec, part_vec, subj_ind, rs_info = ut.build_msc_resting_data(MSC_DIR, f"/{subj_list_file}",
    #                                                 atlas, ses_list=[f'ses-rest{i}' for i in range(1, global_counter+1)],
    #                                                 join_sess=False, join_sess_part=False,
    #                                                 part_ind='run', part_num=None, cond_ind=['net_id'],
    #                                                 type='Ico642Run', hemis=None, smooth='6fwhm_binarized')
    #
    #     # for half in [1,2]:
    #     data = data1 + data
    ...
    #                                                                 'parcel_specific_kappa': False})
    #             Pindiv = pt.argmax(U_indiv, dim=1) + 1
    #             np.save(MODEL_DIR + f'/Models_03/indiv_parcellation/MSC_test_set/prob' +
    #                     f'/asym_MdNiIbHc+MSCrest-1run{global_counter}+task-1run{global_counter}-indiv_space-fs32k_K-17_Ico642Run_groupstrengh-{p}_spatial-{w}.npy',
    #                     U_indiv.cpu().numpy())
    #
    #             del M
    #             pt.cuda.empty_cache()
    ...
    #
    #             ## Save indiv parcellation in cifti
    #             T = pd.read_csv(MSC_DIR + f"/{subj_list_file}", sep='\t')
    #             img = nt.make_label_cifti(Pindiv.T.cpu().numpy(), atlas.get_brain_model_axis(),
    #                                     column_names=[f'{i}' for i in T.participant_id],
    #                                     label_names=net_name, label_RGBA=colors)
    #             nb.save(img, MODEL_DIR + f'/Models_03/indiv_parcellation/MSC_test_set' +
    #                     f'/asym_MdNiIbHc+MSCrest-1run1+task-1run1-indiv_space-fs32k_K-17_Ico642Run_groupstrengh-{p}_spatial-{w}.dlabel.nii')

    ## Combine all test subjects parcellation
    # indiv_par = []
    # for i in [1, 2, 3, 4]:
    #     par = nb.load(MODEL_DIR + f'/Models_03/indiv_parcellation/HCP200_test_set' +
    #                 f'/asym_MdNiIbHc+HCPrest-1run1+task-half2-indiv_space-fs32k_K-17_Ico642Run_groupstrengh-1_spatial-1_{i}.dlabel.nii').get_fdata()[:]
    ...
    T = pd.read_csv(MSC_DIR + f"/participants.tsv", sep='\t')
    t_data, t_info = ut.load_msc_contrasts('MSC', sess='all', subj=None, smooth=2.55)
    ...
    ## 3. MSC task betas
    t_data, _, _, _, t_info = ut.build_dataset('MSC', atlas=atlas.name,
                                               sess=msc_tasks, cond_ind='reg_id',
                                               type='CondRun', part_ind='sn', subj=None,
                                               part_num=np.arange(2, 11), join_sess=False,
                                               join_sess_part=False, smooth=None)
    t_data = [t_data[1]]
    ...
    for counter_p, p in enumerate(group_strength_list):
        for counter_w, w in enumerate(spatial_list):
            ...
            Pindiv = nb.load(MODEL_DIR + f'/Models_03/indiv_parcellation/validation_set' +
                    f'/asym_MdNiIbHc+HCPrest-run2+task-half1-indiv_space-fs32k_K-17_Ico642Run_groupstrengh-{p}_spatial-{w}.dlabel.nii').get_fdata()[:]
                ...
                for task in tasks_list:
                    if task == 'all':
                        ...
                        np.save(MODEL_DIR + f'/Models_03/indiv_parcellation/validation_set/zvalues' +
                                f'/zvalue_indiv_asym_MdNiIbHc+HCPrest-run2+task-half1-indiv_K-{K}_strengh-{p}_spatial-{w}_sm6.npy',
                        ...
                        np.save(MODEL_DIR + f'/Models_03/indiv_parcellation/validation_set/inhomogeneity' +
                                f'/inhomo_nets_asym_MdNiIbHc+HCPrest-run2+task-half1-indiv_K-{K}_strengh-{p}_spatial-{w}_sm6.npy',
