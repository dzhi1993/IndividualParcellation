...
def batch_fit(datasets, sess,
    ...
    # cond_vec = [np.tile(np.arange(1,1211), 4)]
    # part_vec = [np.repeat(np.array([1,2,3,4]), 1210)]
    # subj_ind = [np.arange(800)]
...
def run_fit_all(set_index):
    """ Function to execute fit_all for a given set index. """
    ...
    fname = fname + f'_HCP{num_subj}subjects_Ico642Run_desc-sm4fwhm_binarized_set{set_index}'
    info.to_csv(wdir + fname + '.tsv', sep='\t')
    with open(wdir + fname + '.pickle', 'wb') as file:
        pickle.dump(models, file)

    return fname  # Returning processed filename for debugging
...
if __name__ == "__main__":
    ...
    # if len(sys.argv) != 4:
    #     print("Usage: python group_fusion.py <i> <num_subj> <num_cpus>")
    #     sys.exit(1)
    ...
    # # Generate tasks for parallel processing (Example: process multiple set indices)
    # set_indices = list(range(num_cpus))  # Modify based on the number of parallel tasks

    # # Set up multiprocessing Pool
    # with Pool(num_cpus) as pool:
    #     results = pool.map(run_fit_all, set_indices)  # Distribute tasks across CPUs
